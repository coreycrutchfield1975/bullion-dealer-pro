...
function renderGold(){
 ...
 <div class="page">
  ...
  <div class="grid one" style="margin-top:12px">
   <section class="card intel-card"><div class="card-head">ALL GOLDBACK DENOMINATIONS</div><div class="card-body">
    <img class="card-bg" src="/img/newlook/card-goldbacks.png" alt="">
    <div class="trend-overlay"><div class="goldback-strip">${GOLDBACK_DENOMS.map(d=>`<div class="image-tile"><img src="/img/newlook/card-goldbacks.png" alt="${d.label} Goldback"><b>${d.label} GB</b><small>${fmt(d.n*gbPreviewRate)}</small></div>`).join('')}</div><button class="module-open" onclick="go('goldbacks')">FULL GOLDBACK CENTER →</button></div>
   </div></section>
  </div>
 </div>`;
}
...
function renderSilver(){
 ...
 const coins=[
  ['Barber / Mercury / Roosevelt Dime (90%)',0.07234],
  ['Barber / Standing Liberty / Washington Quarter (90%)',0.18084],
  ['Barber / Walking Liberty / Franklin Half (90%)',0.36169],
  ['Kennedy Half — 1964 (90%)',0.36169],
  ['Kennedy Half — 1965–1970 (40%)',0.14792],
  ['War Nickel — 1942–1945 (35%)',0.05626],
  ['Morgan / Peace Silver Dollar (90%)',0.77344],
  ['Eisenhower Silver Dollar — collector issue (40%)',0.31610]
 ];
 const rolls=[
  ['90% Dime Roll',50,0.07234],['90% Quarter Roll',40,0.18084],
  ['90% Half Dollar Roll',20,0.36169],['40% Kennedy Half Roll',20,0.14792],
  ['35% War Nickel Roll',40,0.05626],['Morgan / Peace Dollar Roll',20,0.77344],
  ['40% Eisenhower Dollar Roll',20,0.31610]
 ];
...
function multiCalc(){
 ...
 const grams=raw*Number(UNIT_TO_GRAMS[unit]||31.1034768);
 let melt;
 if(metal==='XCU'){
  ...
  <div class="grid two markets-main">
   <section class="card intel-card" id="marketTrend"><div class="card-head">PRECIOUS METALS TREND</div><div class="card-body">
    ...
    <div class="trend-overlay">
     ...
     <div class="market-range-note">CURRENT SESSION · Previous close to latest available price</div>
    </div>
   </div></section>
...
const COIN_LIBRARY=[
 ...
 {id:'silver-eagle',name:'American Silver Eagle',metal:'Silver',oz:1,img:'silver-icon.png',root:true,note:'1 troy oz silver bullion reference'},
...
function syncSlotForKey(key){return key===INV_KEY?'inventory':key===ALERT_KEY?'alerts':key===TYPESET_KEY?'typesets':key===SLAB_KEY?'slabs':null}
...
const COLLECTION_CATALOG=[
 {id:'indian',title:'Indian Head Cents',subtitle:'1859–1909 date set',source:'indian',art:'/img/collections/collection-indian-head-cent.jpg'},
 {id:'lincoln',title:'Lincoln Cents',subtitle:'Key types and issues',source:'lincoln',art:'/img/collections/collection-lincoln-cent.jpg'},
 {id:'buffalo',title:'Buffalo Nickels',subtitle:'64-coin date & mint set',source:'buffalo',art:'/img/newlook/collection-buffalo-nickels-v334.png'},
 {id:'mercury',title:'Mercury Dimes',subtitle:'Key dates & mint issues',source:'mercury',art:'/img/collections/collection-mercury-dime.jpg'},
 {id:'morgan',title:'Morgan Dollars',subtitle:'Mint-by-mint collector set',source:'morgan',art:'/img/collections/collection-morgan-dollar.jpg'},
 {id:'peace',title:'Peace Dollars',subtitle:'1921–1935 date & mint set',source:'peace',art:'/img/collections/collection-peace-dollar.jpg'},
 {id:'eagles',title:'American Eagles',subtitle:'Gold Eagle denominations',source:'gae',art:'/img/newlook/gold-coin-eagle.png'},
 {id:'typeset',title:'U.S. Type Set',subtitle:'20th-century designs',source:'20th',art:'/img/newlook/collection-library-v335.png'},
 {id:'world',title:'World Coins',subtitle:'Custom collecting goals',source:'world',art:'/img/collections/collection-world-coins.jpg'},
 {id:'bullion',title:'Bullion Stack',subtitle:'Custom stack goals',source:'bullion',art:'/img/newlook/card-gold.png'}
];
...
function collectionRecords(setId,coinId){return loadJson(SLAB_KEY,[]).filter(x=>x.setId===setId&&x.coinId===coinId)}
function collectionRecordTotals(setId){return loadJson(SLAB_KEY,[]).filter(x=>x.setId===setId).reduce((a,x)=>{a.count++;a.cost+=Math.max(0,Number(x.cost||0));a.market+=Math.max(0,Number(x.marketValue||0));return a},{count:0,cost:0,market:0})}
function drawCollectionCards(){const host=document.querySelector('#collectionStarterGrid');if(!host)return;host.innerHTML=COLLECTION_CATALOG.map(c=>{const p=collectionProgress(c.id);return `<button class="collection-library-card" style="--collection-art:url('${c.art}')" onclick="openCollection('${c.id}')"><span><small>${c.subtitle}</small><b>${c.title}</b><em>${p.have} / ${p.total} · ${p.pct}%</em><i><u style="width:${p.pct}%"></u></i></span></button>`}).join('')}
...
function drawCollectionDetail(){
 ...
 const owned=collectionOwned(activeCollection),p=collectionProgress(activeCollection),sections=set.sections||[],totals=collectionRecordTotals(activeCollection);
 const coin=(x,advanced=false)=>{const records=collectionRecords(activeCollection,x.id);return `<button class="collection-coin ${owned[x.id]?'owned':''} ${advanced?'key':''}" onclick="openCoinRecord('${activeCollection}','${x.id}')"><i>${owned[x.id]?'✓':''}</i><span><b>${x.name||x.label}</b><small>${records.length?`${records.length} RECORDED · `:''}${x.note||x.years||'REGULAR ISSUE'}</small></span></button>`};
 host.innerHTML=`<div class="collection-detail-hero" style="--detail-art:url('${cat.art}')"><div><button onclick="closeCollection()">← ALL COLLECTIONS</button><small>${cat.subtitle}</small><h2>${set.title}</h2><p>${set.desc||''}</p></div></div><div class="buffalo-collection-stats"><div><strong>${p.have} <span>/ ${p.total}</span></strong><small>ISSUES OWNED</small></div><div class="collection-progress"><i style="width:${p.pct}%"></i></div><b>${p.pct}% COMPLETE</b><span>${p.total-p.have} remaining</span></div><div class="collection-value-strip"><div><small>DETAILED EXAMPLES</small><b>${totals.count}</b></div><div><small>PURCHASE COST</small><b>${fmt(totals.cost)}</b></div><div><small>COLLECTOR VALUE</small><b>${fmt(totals.market)}</b></div><p>Collector value is user-entered and remains separate from bullion melt value.</p></div><div class="buffalo-checklist"><div class="collection-check-head"><div><small>COLLECTION CHECKLIST</small><h3>${p.total} tracked issues</h3></div><div><button onclick="exportCollectionRecords()">RECORDS CSV ↓</button><button onclick="exportShoppingList()">SHOPPING LIST ↓</button><button class="${collectionFilter==='all'?'active':''}" onclick="setCollectionFilter('all')">ALL</button><button class="${collectionFilter==='missing'?'active':''}" onclick="setCollectionFilter('missing')">MISSING ONLY</button></div></div>${sections.map(s=>`<h4>${s.heading}</h4><div class="collection-coin-grid">${(s.coins||[]).filter(x=>collectionFilter!=='missing'||!owned[x.id]).map(x=>coin(x)).join('')||'<p class="collection-complete">Section complete.</p>'}</div>`).join('')}${set.varieties?`<h4>ADVANCED VARIETIES <span>Tracked separately from regular-set completion</span></h4><div class="collection-coin-grid advanced">${set.varieties.filter(x=>collectionFilter!=='missing'||!owned[x.id]).map(x=>coin(x,true)).join('')}</div>`:''}</div><div id="coinRecordDrawer"></div>`;
}
function collectionCoin(setId,coinId){const set=collectionSet(setId);return [...flatCollectionCoins(set),...(set?.varieties||[])].find(x=>x.id===coinId)}
function certLookupUrl(service,cert){const value=encodeURIComponent(String(cert||'').trim());if(!value)return '';if(service==='PCGS')return `https://www.pcgs.com/cert/${value}`;if(service==='NGC')return `https://www.ngccoin.com/certlookup/${value}/`;return ''}
function openCoinRecord(setId,coinId){
 const host=document.querySelector('#coinRecordDrawer'),coin=collectionCoin(setId,coinId);if(!host||!coin)return;
 const records=collectionRecords(setId,coinId),owned=Boolean(collectionOwned(setId)[coinId]);
 const rows=records.map((r,i)=>{const lookup=certLookupUrl(r.service,r.cert);return `<article><div><b>${escapeHtml(r.grade||'Ungraded')} ${escapeHtml(r.service||'Raw')}</b><span>${r.cert?`Cert ${escapeHtml(r.cert)}`:'No certification number'}</span></div><div><small>COST</small><strong>${fmt(r.cost||0)}</strong></div><div><small>COLLECTOR VALUE</small><strong>${fmt(r.marketValue||0)}</strong></div>${lookup?`<a href="${lookup}" target="_blank" rel="noopener">VERIFY ↗</a>`:''}<button onclick="removeCoinRecord('${setId}','${coinId}','${r.id}')">×</button></article>`}).join('');
 host.innerHTML=`<div class="coin-record-backdrop" onclick="closeCoinRecord(event)"><section class="coin-record-panel" role="dialog" aria-modal="true" aria-label="Coin record"><header><div><small>COLLECTION RECORD</small><h3>${escapeHtml(coin.name||coin.label)}</h3><p>${escapeHtml(coin.note||coin.years||'Regular issue')}</p></div><button onclick="closeCoinRecord()">×</button></header><div class="coin-owned-toggle"><button class="${owned?'active':''}" onclick="toggleCollectionIssue('${setId}','${coinId}');openCoinRecord('${setId}','${coinId}')">${owned?'✓ OWNED':'MARK AS OWNED'}</button><span>Add every example you own below; duplicates are retained separately.</span></div>${rows?`<div class="coin-record-list">${rows}</div>`:'<div class="coin-record-empty">No detailed examples recorded yet.</div>'}<form onsubmit="saveCoinRecord(event,'${setId}','${coinId}')"><h4>ADD AN EXAMPLE</h4><div class="form-row"><div class="field"><label>HOLDER</label><select id="crService"><option value="Raw">Raw / uncertified</option><option value="PCGS">PCGS</option><option value="NGC">NGC</option><option value="CACG">CACG</option><option value="ANACS">ANACS</option><option value="ICG">ICG</option></select></div><div class="field"><label>GRADE</label><input id="crGrade" maxlength="24" placeholder="MS-63, VF-20, Proof"></div></div><div class="form-row"><div class="field"><label>CERTIFICATION NUMBER</label><input id="crCert" maxlength="48" autocomplete="off" placeholder="Optional"></div><div class="field"><label>PURCHASE DATE</label><input id="crDate" type="date"></div></div><div class="form-row"><div class="field"><label>PURCHASE COST</label><input id="crCost" type="number" min="0" step=".01" placeholder="0.00"></div><div class="field"><label>COLLECTOR / MARKET VALUE</label><input id="crMarket" type="number" min="0" step=".01" placeholder="Separate from melt value"></div></div><div class="field"><label>NOTES</label><textarea id="crNotes" maxlength="500" placeholder="Dealer, provenance, toning, condition, location…"></textarea></div><button class="gold-btn" type="submit">SAVE COIN RECORD</button></form></section></div>`;
 document.body.classList.add('record-open');
}
function closeCoinRecord(event){if(event&&event.target!==event.currentTarget)return;document.querySelector('#coinRecordDrawer').innerHTML='';document.body.classList.remove('record-open')}
function saveCoinRecord(event,setId,coinId){
 event.preventDefault();if(!inventoryEntitled())return;const records=loadJson(SLAB_KEY,[]),service=document.querySelector('#crService').value;
 records.push({id:`cr_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,setId,coinId,service,grade:document.querySelector('#crGrade').value.trim(),cert:document.querySelector('#crCert').value.trim(),purchaseDate:document.querySelector('#crDate').value,cost:Math.max(0,Number(document.querySelector('#crCost').value||0)),marketValue:Math.max(0,Number(document.querySelector('#crMarket').value||0)),notes:document.querySelector('#crNotes').value.trim(),createdAt:new Date().toISOString()});
 saveJson(SLAB_KEY,records);const owned=collectionOwned(setId);if(!owned[coinId])toggleCollectionIssue(setId,coinId);else{drawCollectionDetail();drawCollectionCards()}setTimeout(()=>openCoinRecord(setId,coinId),0);
}
function removeCoinRecord(setId,coinId,recordId){if(!inventoryEntitled())return;saveJson(SLAB_KEY,loadJson(SLAB_KEY,[]).filter(x=>x.id!==recordId));drawCollectionDetail();drawCollectionCards();setTimeout(()=>openCoinRecord(setId,coinId),0)}
function exportShoppingList(){
 const set=collectionSet(activeCollection),owned=collectionOwned(activeCollection);if(!set)return;const rows=[['Collection','Section','Issue','Details','Priority']];
 (set.sections||[]).forEach(s=>(s.coins||[]).filter(x=>!owned[x.id]).forEach(x=>rows.push([set.title,s.heading,x.name||x.label,x.note||x.years||'',/key|semi-key/i.test(x.note||'')?'Key / Semi-key':'Regular'])));
 if(rows.length===1)return;const csv=rows.map(r=>r.map(csvCell).join(',')).join('\n'),url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'})),a=document.createElement('a');a.href=url;a.download=`BDP-${activeCollection}-shopping-list-${new Date().toISOString().slice(0,10)}.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
}
function exportCollectionRecords(){
 const set=collectionSet(activeCollection),records=loadJson(SLAB_KEY,[]).filter(x=>x.setId===activeCollection);if(!set||!records.length)return;const coins=[...flatCollectionCoins(set),...(set.varieties||[])],rows=[['Collection','Issue','Holder','Grade','Certification','Purchase Date','Cost','Collector Value','Notes']];records.forEach(r=>{const coin=coins.find(x=>x.id===r.coinId);rows.push([set.title,coin?.name||coin?.label||r.coinId,r.service,r.grade,r.cert,r.purchaseDate,Number(r.cost||0).toFixed(2),Number(r.marketValue||0).toFixed(2),r.notes])});const csv=rows.map(r=>r.map(csvCell).join(',')).join('\n'),url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'})),a=document.createElement('a');a.href=url;a.download=`BDP-${activeCollection}-collection-records-${new Date().toISOString().slice(0,10)}.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
}
...
const NEWS_SOURCES=[
 ...
 {name:'U.S. MINT',desc:'Official coin specifications, programs and product information.',url:'https://www.usmint.gov/',art:'card-coins.png',tag:'COINS'},
 {name:'COINFLATION',desc:'U.S. coin melt values and metal-content reference.',url:'https://www.coinflation.com/',art:'card-silver.png',tag:'COINS'},
 {name:'LBMA',desc:'Global precious-metals market standards and reference material.',url:'https://www.lbma.org.uk/',art:'card-market.png',tag:'MARKETS'},
 {name:'CME GROUP',desc:'Metals futures specifications, calendars and market education.',url:'https://www.cmegroup.com/markets/metals.html',art:'mkt-gold.png',tag:'MARKETS'},
 {name:'NUMISTA',desc:'World coin catalog, specifications and collection reference.',url:'https://en.numista.com/',art:'coins-hero.png',tag:'COINS'}
];
const RESOURCE_GROUPS=[
 {title:'PRECIOUS METALS BASICS',desc:'Live metal values, purity math and practical melt-value tools.',icon:'i-bars',items:[['Gold Center','gold'],['Silver Center','silver'],['Live Markets','markets'],['Calculator Center','calculators']]},
 {title:'COINS & GOLDBACKS',desc:'Identify issues, compare intrinsic value and reach trusted attribution sources.',icon:'i-coin',items:[['Coin Center','coins'],['Goldback Center','goldbacks'],['Numismatic Sources','coins'],['Goldback Reference','goldbacks']]},
 {title:'DEALER REFERENCE',desc:'Move from market price to a documented buy, sell or inventory decision.',icon:'i-tools',items:[['Dealer Tools','dealer'],['Inventory','inventory'],['Price Alerts','alerts'],['Calculator Center','calculators']]},
 {title:'MARKET REFERENCE',desc:'Follow ratios, futures context, market sessions and specialist reporting.',icon:'i-chart',items:[['Market Ratios','markets'],['Dealer / Trading Tools','dealer'],['Live Calculator','calculators'],['News Sources','news']]}
];
...
function renderResources(){
 ...
 <div class="page">
  ...
  <div class="resource-groups">${RESOURCE_GROUPS.map(g=>`<section class="resource-group"><div class="resource-group-head">${icon(g.icon)}<h3>${g.title}</h3></div><p>${g.desc}</p><div class="resource-links">${g.items.map(i=>`<button onclick="go('${i[1]}')">${i[0]}<span>→</span></button>`).join('')}</div></section>`).join('')}</div>
  ...
  <div class="grid one" style="margin-top:12px">
   <section class="card intel-card"><div class="card-head">BUILD A RESEARCH WORKFLOW</div><div class="card-body">
    <img class="card-bg" src="/img/newlook/card-dealerboard.png" alt="">
    <div class="trend-overlay"><div class="resource-workflow"><button onclick="go('markets')"><b>1</b><span>CHECK LIVE MARKET</span></button><button onclick="go('calculators')"><b>2</b><span>CALCULATE METAL VALUE</span></button><button onclick="go('coins')"><b>3</b><span>RESEARCH THE COIN</span></button><button onclick="go('inventory')"><b>4</b><span>SAVE THE HOLDING</span></button></div></div>
   </div></section>
  </div>
 </div>`;
}
