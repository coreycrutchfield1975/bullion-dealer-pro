...
app.post('/api/stripe/webhook', (req, res) => {
  ...
  if (event.type === '[checkout.session.com](https://checkout.session.com)pleted') {
    ...
    const email = (sess.customer_email || [sess.me](https://sess.me)tadata?.email || '').toLowerCase();
    const user = users.get(email);
    if (user) { user.stripeCustomerId = sess.customer; user.stripeSubId = sess.subscription; user.plan = 'monthly'; users.set(email, user); }
  }
  if (event.type === 'invoice.paid') {
    const inv = event.data.object;
    for (const [, u] of users) {
      if (u.stripeCustomerId === inv.customer) { u.plan = u.plan === 'annual' ? 'annual' : 'monthly'; users.set(u.email, u); break; }
    }
  }
  if (event.type === 'customer.subscription.deleted') {
    const sub = event.data.object;
    for (const [, u] of users) {
      if (u.stripeCustomerId === sub.customer) { u.plan = 'trial'; users.set(u.email, u); break; }
    }
  }
  res.sendStatus(200);
});
...
app.get('/api/admin/stats', adminMiddleware, (req, res) => {
  ...
  res.json({ total: all.length, trial: all.filter(u=>u.plan==='trial').length, monthly: all.filter(u=>u.plan==='monthly').length, annual: all.filter(u=>u.plan==='annual').length, users: all.map(({passwordHash,...u})=>u) });
});
...
app.post('/api/admin/set-plan', adminMiddleware, (req, res) => {
  ...
  user.plan = plan; users.set(user.email, user);
  res.json({ ok: true });
});
...
app.get('/api/access', authMiddleware, (req, res) => {
  ...
  res.json({ plan: user.plan, access: hasPaidAccess(user) || (user.plan==='trial'&&!trialExpired), trialEnd: user.trialEnd||null, trialExpired });
});
