...
function getAppUrl() {
  return process.env['APP' + '_URL'] || ('http://localhost:' + PORT);
}
...
async function seedAdmin() {
  const adminEmail = (process.env['ADMIN' + '_EMAIL'] || '').toLowerCase().trim();
  const adminPassword = (process.env['ADMIN' + '_PASSWORD'] || '').trim();
...
app.use(cors({ origin: getAppUrl(), credentials: true }));
...
app.post('/api/stripe/checkout', authMiddleware, async (req, res) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: getAppUrl() + '/app?session_id={CHECKOUT_SESSION_ID}',
...
app.post('/api/stripe/webhook', (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const email = (sess.customer_email || (sess.metadata && sess.metadata.email) || '').toLowerCase();
    const user = users.get(email);
    if (user) {
